...
def set_data(url, db, board_name):
    ...
    target_context = '※ 發信站: 批踢踢實業坊(ptt.cc),'
...
def word_freq(board_name, db):
    ...
    query = art_ref.order_by('date', direction=firestore.Query.DESCENDING).limit(1000) #取1000篇
    results = query.get()
    ...
    doc_ref = db.collection('big_analyze').document(board_name)
...
if __name__ == '__main__':
    ...
    #board_iter('Baseball', db, 5000)
    word_freq('Baseball',db)
    #board_iter('Stock', db, 3000)
    word_freq('Stock',db)
    #board_iter('LoL', db, 3000)
    word_freq('LoL',db)
    #board_iter('C_Chat', db, 3000)
    word_freq('C_Chat',db)
    #board_iter('Gossiping', db, 30000)
    word_freq('Gossiping',db)

    time_now = datetime.now()
    time_now = pytz.timezone('Asia/Taipei').localize(time_now)
    time_update = {'update_time' : time_now}
    doc_ref = db.collection('database_updatetime').document('update_time')
